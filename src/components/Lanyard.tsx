...
// The card model's front face ist UV-mapped to the LEFT half of the texture
...
function Band({
  ...
  const { nodes, materials } = useGLTF(cardGLB) as unknown as { nodes: { card: { geometry: THREE.BufferGeometry }; clip: { geometry: THREE.BufferGeometry }; clamp: { geometry: THREE.BufferGeometry } }; materials: { base: { map: THREE.Texture }; metal: THREE.Material } };
  ...
  const cardMap = useMemo(() => {
    ...
    if (frontImage && frontTex.image) drawFitted(frontTex.image as HTMLImageElement | HTMLCanvasElement, FRONT_UV_RECT);
    if (backImage && backTex.image) drawFitted(backTex.image as HTMLImageElement | HTMLCanvasElement, BACK_UV_RECT);
