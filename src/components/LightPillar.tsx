...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  if (!webGLSupported) {
    return (
      <div
        className={`absolute inset-0 flex items-center justify-center bg-black/10 text-sm text-gray-500 ${className}`}
        style={{ mixBlendMode }}
      >
        WebGL not supported
      </div>
  ...
  return <div ref={containerRef} className={`absolute inset-0 h-full w-full ${className}`} style={{ mixBlendMode }} />;
};
